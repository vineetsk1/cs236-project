...
class ModelD(nn.Module):
    ...
    def forward(self, x, labels):
        batch_size = x.size(0)
        x = x.view(batch_size, 1, 28,28)       #MODIFY THIS TO MODIFY SIZE OF IMAGES
        x = self.conv1(x)
        x = self.bn1(x)
        x = F.relu(x)
        x = self.conv2(x)
        x = self.bn2(x)
        x = F.relu(x)
        x = x.view(batch_size, 64*28*28)        #MODIFY THIS TO MODIFY SIZE OF IMAGES
        y_ = self.fc3(labels)
        y_ = F.relu(y_)
        x = torch.cat([x, y_], 1)
        x = self.fc1(x)
...
class ModelG(nn.Module):
    def __init__(self, z_dim):
        ...
        self.fc = nn.Linear(self.z_dim+1000, 64*28*28)      #MODIFY THIS TO MODIFY SIZE OF IMAGES
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nz', type=int, default=100,
                        help='Number of dimensions for ins noise.')
    ...
    train_loader = data_loader(args.dataset_folder, train_dir, args.batch_size)
    ...
    ins = torch.FloatTensor(args.batch_size, ins_SIZE)
    ...
    for epoch_idx in range(args.epochs):
        ...
        for batch_idx, (train_x, train_y) in enumerate(train_loader):
            ...
            train_x = train_x.view(-1, ins_SIZE)
            if args.cuda:
                train_x = train_x.cuda()
                train_y = train_y.cuda()

            ins.resize_as_(train_x).copy_(train_x)
            ...
            output = model_d(insv, Variable(one_hot_labels))
            optim_d.zero_grad()
